...
void readClusters(int nEvents = 1, bool leadRun = false) {
  ...
  (canvas[0]).reset(new TCanvas(Form("MIP Cluster-Charge %i", fname),
                                Form("MIP Cluster-Charge %i", fname), 1200,
                                1200));

  (canvas[1]).reset(new TCanvas(Form("Digit Charge Log%i", fname),
                                Form("Digit Charge Log %i", fname), 1200,
                                1200));
  (canvas[1])->SetLogy();
  (canvas[1])->SetLogx();

  (canvas[2]).reset(new TCanvas(Form("Digit-Map %i", fname),
                                Form("Digit-Map %i", fname), 1200, 1200));

  (canvas[3]).reset(new TCanvas(
      Form("Digit-Map Average (Total Events) %i", fname),
      Form("Digit-Map Average (Total Events) %i", fname), 1200, 1200));

  (canvas[4]).reset(new TCanvas(Form("Chamber Occupancy %i", fname),
                                Form("Chamber Occupancy %i", fname), 1200,
                                1200));
  (canvas[4])->SetLogy();

  (canvas[5]).reset(new TCanvas(Form("PlaceHolder %i", fname),
                                Form("PlaceHolder %i", fname), 1200, 1200));
  (canvas[6]).reset(new TCanvas(Form("PlaceHolder2 %i", fname),
                                Form("PlaceHolder2 %i", fname), 1200, 1200));
  (canvas[7]).reset(new TCanvas(Form("PlaceHolder3 %i", fname),
                                Form("PlaceHolder3 %i", fname), 1200, 1200));
  ...
  tempX.reset(new TCanvas(Form("Event Channel Average %i", fname),
                          Form("Event Channel Average %i", fname), 1200, 2000));
  tempX->Divide(3, 3);
  tempX->cd(3);
  tpvs2[4]->Draw();

  tempY.reset(new TCanvas(Form("Event Channel Count %i", fname),
                          Form("Event Channel Count %i", fname), 1200, 2000));
  tempY->Divide(3, 3);
  tempY->cd(3);
  tpvs2[5]->Draw();
  ...
  temp1.reset(new TCanvas(Form("Event Information %i", fname),
                          Form("Event Information %i", fname), 1200, 2000));
  temp1->Divide(2, 2);
  temp1->cd(2);
  tpvs2[0]->Draw();
  ...
  */
  ...
  temp2.reset(new TCanvas(Form("Trigger Frequency%i", fname),
                          Form("Trigger Frequency%i", fname), 1200, 2000));
  temp2->Divide(2, 2);
  // temp2->cd(2);
  // tpvs2[1]->Draw();
  ...
  triggerTimeFreqHist[i]->SetTitleOffset(triggerTimeFreqHist[i]->GetTitleOffset("y")*1.2,
  "xy");
  triggerTimeFreqHist[i]->SetTitleSize(triggerTimeFreqHist[i]->GetTitleSize("x")*0.95,
  "xy");
  triggerTimeFreqHist[i]->SetLabelSize(triggerTimeFreqHist[i]->GetLabelSize("x")*0.925,
  "x");
  triggerTimeFreqHist[i]->SetLabelSize(triggerTimeFreqHist[i]->GetLabelSize("y")*0.925,
  "y");*/
  ...
  digMap0.reset(new TCanvas(Form("Digits With 0 - charge%i", fname),
                            Form("Digits With 0 - charge%i", fname), 1200,
                            1200));
  digMap0->Divide(3, 3);
  ...
  digMapSelCanv.reset(new TCanvas(Form("Pads turned off by user%i", fname),
                                  Form("Pads turned off by user%i", fname),
                                  1200, 1200));
  ...
  digMapLowCan.reset(new TCanvas(Form("Charge below 4 %i", fname),
                                 Form("Charge Below 4 %i", fname), 1200, 1200));
  ...
  t.reset(new TCanvas(Form("Digits SmallRange%i", fname),
                      Form("Digits SmallRange %i", fname), 1200, 1200));
  t->Divide(3, 3);
  t->cd(3);
  (tpvs.back())->Draw();
