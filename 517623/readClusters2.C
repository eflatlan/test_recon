...
void readClusters(int nEvents) 
{
  ...
  //digPerEvent[i].reset(new TH1F(digEvtFreqStr, digEvtFreqStr, 500, 100*(avgDigits-150.)/(144*160), 100*(avgDigits+150.)/(144*160)));
    ...
    digPerEvent[i]->SetYTitle("Frequency");
  ...
  (canvas[5]).reset(new TCanvas(Form("Digits Per 2Event %i",fname), Form("Digits Per2 Event %i",fname), 1200, 1200));

  (canvas[6]).reset(new TCanvas(Form("Digits Per 3Event %i",fname), Form("Digits 3Per Event %i",fname), 1200, 1200));
  ...
  Int_t nTotTriggers = 0;
    ...
    for(const auto& dig : digits){
      ...
      digMapAvg[module]->Fill(padChX, padChY, dig.getQ()/numTriggers);//numTriggers
  ...
  for (int iCh = 0; iCh < 7; iCh++) {
    ...
    const auto maxPos2 = digPerEvent[iCh]->GetMaximumBin();
    cout << "maxpos2 " << maxPos2 << endl;
