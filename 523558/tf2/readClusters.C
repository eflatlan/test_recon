...
void readClusters(int nEvents) 
{
  ...
  if(!fileFound){
    cout << "No fitting file found!" << endl << "Exiting " << endl; 
    return;
    std::exit(0);
  }
...
void initFileIn(const std::string &filename) {
  ...
  if (!mTree) {
    ...
    cout << "HMPID DigitToClusterSpec::init() : Did not find o2sim tree in "
        << filename.c_str() << endl << "Exiting " << endl; 
    return;
    std::exit(0);
  }
  ...
  if ((mTree->GetBranchStatus("HMPDigit")) == 1) {
    ...
    cout << "No Branch found!" << endl << "Exiting " << endl; 
    return;
    std::exit(0);
  }
