...
{ 
  ...
  auto runNumber = (gwd.substr(gwd.length()-9)).c_str();
  Printf("gwd %s",gwd.c_str());
  Printf("pwd %s",gSystem->pwd());
  Printf("homedir %s",gSystem->HomeDirectory());
  Printf("Empty clusters");

  TCanvas *c1 = new TCanvas("c1",Form("Cluster-Map %s", runNumber),2000,1200); 	
  TCanvas *c2 = new TCanvas("c2",Form("Cluster-Charge %s", runNumber),2000,1200); 	
  TCanvas *c3 = new TCanvas("c3",Form("MIP Cluster-Charge %s", runNumber),2000,1200); 	
  TCanvas *c4 = new TCanvas("c4",Form("Cluster-Size %s", runNumber),2000,1200);  
  /*
  ...
  c1->SaveAs(Form("clusterMap_%s.eps", runNumber));
  c2->SaveAs(Form("clusterCharge_%s.eps", runNumber));
  c3->SaveAs(Form("mipClustesCharge_%s.eps", runNumber));
  c4->SaveAs(Form("clusterSize_%s.eps", runNumber));
}    
